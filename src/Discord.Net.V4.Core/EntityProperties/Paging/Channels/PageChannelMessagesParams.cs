...
public sealed record PageChannelMessagesParams(
    int? PageSize = null,
    int? Total = null,
    Optional<IdOrEntity<ulong, IMessage>> Around = default,
    Optional<IdOrEntity<ulong, IMessage>> Before = default,
    Optional<IdOrEntity<ulong, IMessage>> After = default
) :
    IPagingParams<Routes.ListMessages, IReadOnlyCollection<IMessageModel>>
{
    public static int MaxPageSize => DiscordConfig.MaxMessagesPerBatch;

    public Routes.ListMessages? GetNext(
        Routes.ListMessages route,
        IReadOnlyCollection<IMessageModel>? previousApiResult)
    {
        if (previousApiResult is null)
        {
            return route with
            {
                Before = Before.MapToId(),
                Around = Around.MapToId(),
                After = After.MapToId()
            };
        }
